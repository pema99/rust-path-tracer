...
pub fn build_light_pick_table(vertices: &[Vec4], indices: &[UVec4], mask: &[bool]) -> Vec<(u32, u32, f32)> {
    ...
    let average_probability =
        triangle_probabilities.iter().sum::<f32>() / total_tris as f32;
    ...
    let table = bins
        ...
        .map(|x| {
            (
                x.index_a as u32,
                x.index_b as u32,
                x.probability_a / (x.probability_a + x.probability_b),
            )
        })
        .collect::<Vec<_>>();
...
// Just for reference
#[allow(dead_code)]
fn pick_light(table: &[(u32, u32, f32)]) -> u32 {
